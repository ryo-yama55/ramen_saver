...
export class LocalStorageSavingsRecordRepository implements ISavingsRecordRepository {
    ...
    try {
      ...
      return (data as StoredSavingsRecord[]).map((record) => {
        ...
        if (isNaN(date.getTime())) {
          console.warn(
            `Invalid date for record ${record.id}: ${record.recordedAt}`
          );
        }
        return {
          ...record,
      ...
      console.error(
        'Failed to parse savings records from localStorage:',
        error
      );
      return [];
    }
  ...
  async findAll(filters?: SavingsRecordFilters): Promise<SavingsRecord[]> {
    ...
    if (filters?.offset !== undefined || filters?.limit !== undefined) {
      ...
      const end =
        filters.limit !== undefined ? start + filters.limit : undefined;
